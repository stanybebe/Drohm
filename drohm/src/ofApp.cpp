...
void ofApp::setup() {
    ...
    // open patch
    Patch patch = pd.openPatch("pd/drohm.pd");
    cout << patch << endl;
...
void ofApp::draw() {
    ...
    xyA.draw(325, 40, 1000, 1000);
    xyB.draw(325, 180, 1000, 1000);
    ...
    pd.sendFloat("vola", knobA.value);
    pd.sendFloat("volb", knobB.value);
    pd.sendFloat("lfoaa", knobC.value);
    pd.sendFloat("lfoab", knobD.value);
    pd.sendFloat("rate", xyA.valueX);
    pd.sendFloat("clock", xyA.valueY);
    pd.sendFloat("lfora", xyB.valueX);
    pd.sendFloat("lforb", xyB.valueY);
